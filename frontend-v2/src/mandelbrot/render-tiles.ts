...
async function renderTiles(
  ...
  const topLeftTile = tiles[0];
  if (!topLeftTile) {
    console.warn('no tiles...???');
    return;
  }
  const topLeftTileCoord = topLeftTile.params.coord;
  ...
  // const cacheHits = tiles.filter((tile) => isInCache(tile)).length;
  ...
  const msg = [
    `^_^ renderTiles took ${(t1 - t0).toFixed(2)} ms.`,
    // `Cache hits: ${cacheHits}/${tiles.length}, Zoom: ${f3(zoom)}`,
  ].join(' ');
